#!/usr/bin/env python3
"""
Filter for 'biomarker discovery' and analyze funding by year
"""
...
def main():
    ...
    # Filter for "biomarker discovery" in key text fields
    print("=" * 80)
    print("FILTERING FOR 'BIOMARKER DISCOVERY'")
    print("=" * 80)
    ...
    for field in text_fields:
        if field in df.columns:
            field_mask = df[field].astype(str).str.lower().str.contains('biomarker discovery', na=False)
            mask = mask | field_mask
            count = field_mask.sum()
            print(f"  Found in {field}: {count:,} projects")

    filtered_df = df[mask]
    print(f"\nTotal projects with 'biomarker discovery': {len(filtered_df):,}")
    ...
    # Top institutes for biomarker discovery
    print("\n" + "=" * 80)
    print("TOP 10 INSTITUTES FOR BIOMARKER DISCOVERY")
    print("=" * 80)
    ...
    output_file = script_dir / "data" / "oct-2024" / "biomarker_discovery_filtered.csv"
