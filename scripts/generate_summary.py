...
def find_filtered_files(filtered_dir: Path, logger: logging.Logger) -> List[Dict]:
    ...
    for file_path in files:
        ...
        try:
            ...
            logger.info(
                f"  FY{year}: {matched_projects:,} projects ({file_path.stat().st_size / 1024 / 1024:.1f} MB)"
            )
        except ValueError:
...
def generate_summary_markdown(
    ...
    with open(output_path, "w") as f:
        ...
        for year_info in year_data:
            try:
                ...
                if "EXPLICIT_BIOMARKER" in df.columns:
                    ...
                    logger.warning(
                        f"FY{year_info['year']}: No EXPLICIT_BIOMARKER column found"
                    )
                ...
                f.write(
                    f"| {year_info['year']} | ${year_funding / 1e9:.2f}B | ${year_explicit_funding / 1e9:.2f}B | {year_info['matched_projects']:,} | {estimated_scanned:,} |\n"
                )
                ...
                f.write(
                    f"| {year_info['year']} | Error | Error | {year_info['matched_projects']:,} | - |\n"
                )
        ...
        if total_explicit > 0:
            ...
            f.write(
                f"- **Other Biomarker-Related Projects**: {total_matched - total_explicit:,}\n"
            )
        f.write(f"- **Total Scanned Projects**: {total_scanned:,}\n")
        f.write(
            f"- **Overall Match Rate**: {(total_matched / total_scanned * 100):.1f}%\n"
        )
        if total_explicit > 0:
            f.write(
                f"- **Explicit Biomarker Rate**: {(total_explicit / total_scanned * 100):.1f}%\n"
            )
        ...
        if total_explicit_funding > 0:
            f.write(
                f"- **Explicit Biomarker Spending**: ${total_explicit_funding / 1e9:.2f}B ({total_explicit_funding / total_funding * 100:.1f}%)\n"
            )

        f.write("\n### Processing\n")
        f.write(f"- **Years Available**: {len(year_data)}\n")
        f.write(
            f"- **Year Range**: {min(y['year'] for y in year_data)}-{max(y['year'] for y in year_data)}\n\n"
        )
        ...
        for year_info in year_data:
            f.write(
                f"- `{year_info['file_path'].name}` (includes `EXPLICIT_BIOMARKER` column)\n"
            )
