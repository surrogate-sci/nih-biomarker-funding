...
def main():
    ...
    clean = [(y, t, e) for y, t, e in zip(YEARS, BIOMARKER_RELEVANT, EXPLICIT_BIOMARKER)
    ...
    ax.fill_between(years, 0, explicit, alpha=0.8, color="#2e86ab",
                     label=f"Explicit biomarker terms (${explicit_sum:.0f}B)")
    ax.fill_between(years, explicit, total, alpha=0.5, color="#d4a574",
    ...
    ax.axhline(y=share_avg, color="#d4574a", linestyle="--", linewidth=1.5, alpha=0.7,
               label=f"Average: {share_avg:.0f}%")

    ax.set_xlabel("NIH Fiscal Year", fontsize=12)
    ax.set_ylabel("% of Biomarker Spending Using\nExplicit Biomarker Terms", fontsize=11)
    ax.set_title(
