...
**`predictive_optimal`** — Assign when the biomarker predicts differential response between two or more named treatments, with evidence or intent to demonstrate a biomarker × treatment interaction. The biomarker enables selection between active therapeutic alternatives. The abstract must name multiple treatments and claim or test comparative response. Distinguish from `predictive_enrichment` by the presence of comparative treatment data or intent; distinguish from `stratification_treatment` by the framing: predictive_optimal focuses on a specific biomarker-treatment interaction, whereas stratification_treatment focuses on subtyping patients into treatment-assignment groups.
...
**`stratification_treatment`** — Assign when the primary aim is to classify patients into subtypes for the purpose of assigning different treatments. The abstract frames subtyping as enabling differential therapeutic decisions across groups. Distinguish from `predictive_optimal` by the scope: stratification_treatment focuses on subtyping the patient population, whereas predictive_optimal focuses on a specific biomarker × treatment interaction.
