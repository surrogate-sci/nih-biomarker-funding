...
**`pharmacodynamic`** — Assign when the biomarker measures a biological response to a specific intervention (drug, therapeutic agent, or experimental compound). The biomarker's role is to quantify the magnitude of biological effect at a given dose or schedule. A pharmacodynamic biomarker does not itself predict clinical outcome; if the abstract claims the biomarker also predicts efficacy or patient benefit, assign the predictive or prognostic code that best fits the efficacy claim, and use `pharmacodynamic` as the secondary code. Distinguish from `target_engagement` by inferential purpose: pharmacodynamic biomarkers characterize the magnitude or dose-response of a biological effect; target engagement biomarkers confirm the intervention reached its intended site of action.

**`target_engagement`** — Assign when the biomarker is used as evidence that an intervention reached and modulated its intended biological target, mechanism, or circuit. Target engagement is upstream of pharmacodynamic evidence and closer to the treatment itself: the study asks "did the intervention engage the intended target?" rather than "what is the magnitude or dose-response of the biological effect?" In traditional small molecule pharmacology, target engagement and pharmacodynamic evidence are separable (e.g., receptor occupancy vs. downstream signaling). In non-pharmacological or next-generation therapy contexts (brain stimulation, gene therapy, psychotherapy), the same measurement construct may serve both purposes — assign `target_engagement` when the primary inferential goal is mechanistic confirmation that the intervention reached its intended site of action; assign `pharmacodynamic` when the design is characterizing dose-response or biological effect magnitude. Note: the presence of a brain stimulation, gene therapy, or other non-pharmacological intervention alone does not trigger this code — a biomarker must be explicitly used to confirm target engagement, not merely to measure treatment outcome.

**`efficacy_biomarker`** — Assign when the biomarker provides evidence of reversal or amelioration of a pathological process in response to an intervention. The biomarker must be used as an index of disease state — measuring change in something treated as reflecting the pathological process — but the disease biomarker's causal role need not be definitively established. In psychiatry and neuroscience, treatment-induced change in a candidate disease biomarker is often used as reverse inference evidence about disease etiology; this constitutes efficacy biomarker use even when the underlying pathophysiology is still being characterized. Distinct from `pharmacodynamic` by disease-directedness: a PD biomarker confirms the drug is having a biological effect; an efficacy biomarker specifically shows that effect is reversing or ameliorating a disease process. Distinct from `surrogate_endpoint` by evidentiary standard: an efficacy biomarker has biological plausibility and disease relevance but lacks the formal statistical or regulatory validation required for endpoint substitution. Note: "surrogate endpoint" or "surrogate intermediate endpoint" language in an abstract does not automatically trigger `surrogate_endpoint` — assign `efficacy_biomarker` when the markers are pre-validation or used for mechanistic inference rather than formal endpoint substitution.
...
| "efficacy biomarker"     | efficacy_biomarker (if disease-directed reversal/amelioration); pharmacodynamic (if generic activity measure) |
| "response biomarker"     | pharmacodynamic or predictive_ambiguous                    |
| "target engagement"      | target_engagement                                          |
| "receptor occupancy"     | target_engagement                                          |
| "biodistribution"        | target_engagement (if confirming delivery to intended site); pharmacodynamic (if dose-response characterization) |
